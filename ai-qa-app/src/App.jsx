...
export default function AIQAApp() {
  ...
  return (
    <div style={{
      ...
      fontFamily: "'DM Sans', sans-serif",
      backgroundImage: "radial-gradient(ellipse 80% 50% at 50% -20%, rgba(10,132,255,0.15), transparent)",
    }}>
      <style>{`
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 15, fontWeight: 700, color: "#64ffda" }}>AI Q&A App</div>
          <div style={{ fontSize: 11, color: "#4a7a6a" }}>Powered by OpenRouter · Ask anything</div>
        </div>
        {messages.length > 0 && (
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "24px 20px", maxWidth: 720, width: "100%", margin: "0 auto" }}>
        {messages.length === 0 && !loading && (
          <div style={{ textAlign: "center", paddingTop: "15vh", color: "#2a5a4a" }}>
            <div style={{ fontSize: 52, marginBottom: 16 }}>✦</div>
            <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 20, color: "#64ffda", marginBottom: 8, fontWeight: 700 }}>
              What do you want to know?
            </div>
            <div style={{ fontSize: 14, color: "#3a6a5a" }}>Type your question and press Enter</div>
          </div>
        )}
        ...
        {error && (
          <div style={{
            textAlign: "center", color: "#ff6b6b", fontSize: 13, padding: "10px",
            background: "rgba(255,107,107,0.08)", borderRadius: 10,
            border: "1px solid rgba(255,107,107,0.2)", marginTop: 8,
          }}>{error}</div>
        )}
        <div ref={bottomRef} />
      </div>

      <div style={{ padding: "16px 20px 24px", background: "rgba(5,13,18,0.9)", borderTop: "1px solid rgba(100,255,218,0.08)" }}>
        <div style={{
          maxWidth: 720, margin: "0 auto", display: "flex", gap: 10, alignItems: "flex-end",
          background: "rgba(255,255,255,0.04)", border: "1px solid rgba(100,255,218,0.2)",
          ...
          <textarea
            ...
            style={{
              ...
              color: "#d4ede6", fontSize: 14.5, fontFamily: "'DM Sans', sans-serif",
          ...
          />
          <button onClick={askQuestion} disabled={!input.trim() || loading} style={{
            background: input.trim() && !loading ? "linear-gradient(135deg, #64ffda, #0a84ff)" : "rgba(255,255,255,0.06)",
            border: "none", borderRadius: 10, width: 38, height: 38,
            cursor: input.trim() && !loading ? "pointer" : "not-allowed",
            display: "flex", alignItems: "center", justifyContent: "center",
            fontSize: 16, flexShrink: 0, color: input.trim() && !loading ? "#050d12" : "#3a6a5a",
          }}>↑</button>
        </div>
      </div>
    </div>
